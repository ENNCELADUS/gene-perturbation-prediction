...
> **Status (2026-09-09):** The seed-0 joint GeneEffect run is trained, tested and baselined: Huber beats the context-blind gene mean by 0.08% and residual correlations trail Tx1 PCA-ridge, a working path and not a result. Its response pathway is diagnosed as non-functional: preparation feeds raw UMI counts to a STATE decoder trained in log space, and no count-space interface variant transfers a response to a held-out cell line. An explicit gene-specific context slope lifts validation residual Pearson from 0.05 to 0.13 and ties PCA8-ridge. Further model decisions use validation; `context_screen_v2` remains a separate, unrun SL track. [`Diagnostics`](docs/results/p1_response_pathway_diagnostics/README.md) · [`Joint result`](docs/results/joint_geneeffect_seed0/README.md) · [`Protocol`](docs/03-geneeffect-protocol.md) · [`Research contract`](docs/01-blueprint.md).
...
- **[2026/09]** **Response pathway diagnosed — the count-space interface is the defect.** Seed-0 P1-A/B/C: the joint backbone fed raw UMI counts to a log-space STATE decoder; its response block uses no perturbation identity, and no count-space interface variant beats no-change on a held-out cell line (pooled LOAO ratios 1.03–17). The released checkpoint in approximately its own space beats no-change on HepG2 and Jurkat, not K562 or HCT116. A gene-specific context slope raises validation residual Pearson 0.05 → 0.13 at three head seeds and ties PCA8-ridge. No context or SL claim. [`Result`](docs/results/p1_response_pathway_diagnostics/README.md) · [`Protocol §8`](docs/03-geneeffect-protocol.md#8-response-pathway-diagnostics-p1).
...
- **Training diagnostics, validation and testing.** Every epoch adds fixed-model `train_eval_*` GeneEffect metrics, per-gene residual SD/error summaries and actual update/replay exposure counts to the validation report. Early stopping and `best.pt` use only minimum `val_geneeffect_loss`. Use `src.evaluate --split train` for checkpoint diagnostics; evaluate test explicitly after selection. Feature ablations use the five `model.head_blocks` flags; removing response from the readout requires disabling both `delta_proj` and `s`.
