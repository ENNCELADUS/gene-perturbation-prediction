...
- **[2026/07]** **Foundation-first refocus after the Bridge-A negative.** With the composition mechanism paused, the near-term work is a **few-shot cross-cell-line GeneEffect backbone**: an accurate, context-conditioned single-gene predictor `F(X_c, c, g)` that both bridges depend on, evaluated leave-one-cell-line-out on the **differentially-essential slice** against copy-K562 / mean / nearest-line / lineage / CCLE-regression baselines, with a **few-shot curve** (accuracy vs. k held-out-line labels). This is the roadmap Phase 3 backbone-transfer exit — reported as backbone transfer, **not** an SL result. Task established; not yet run. See [`docs/04-roadmap.md`](docs/04-roadmap.md) §1.1.
