...
Seed-0 response-pathway diagnostics (2026-09-07 to 09) are closed. A numeric-space
defect in response preparation, since corrected, left the joint backbone's response
block without perturbation-identity use; after the correction every adapted interface
learns its training lines but none transfers a response to a held-out cell line
(leave-one-anchor-out held-out loss ratios to no-change 1.09–26 with intervals
excluding 1). The released checkpoint driven in
...
[Protocol §8](03-geneeffect-protocol.md#8-where-the-model-stalls-response-pathway-diagnostics).