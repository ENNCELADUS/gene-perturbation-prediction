...
print(
    f"Cells with all-zero expression: {zero_rows} / {adata.n_obs} ({100 * zero_rows / adata.n_obs:.2f}%)"
)
...
flat_X = X.flatten()
print(
    f"Percentiles: 0%={np.percentile(flat_X, 0):.4f}, 25%={np.percentile(flat_X, 25):.4f}, 50%={np.percentile(flat_X, 50):.4f}, 75%={np.percentile(flat_X, 75):.4f}, 99%={np.percentile(flat_X, 99):.4f}, 100%={np.percentile(flat_X, 100):.4f}"
)
