...
from aivc_model.tx1_basal import (  # noqa: E402
...
def _build_22rv1(
    ...
    *,
    ...
    with h5py.File(source) as handle:
        ...
        mask = (col_data["Cell"] == b"22Rv1") & (
            col_data["TREATMENT"] == b"22Rv1-DMSO"
        )
    ...
    *,
    ...
    for model_id, info in _registered_specials(registry_path).items():
        ...
        if model_id in {"ACH-000696", "ACH-000779", "ACH-001086"}:
            result = _copy_registered_h5ad(
                source, info, max_cells=max_cells, seed=seed
            )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--tahoe-gene-metadata",
        type=Path,
        default=Path("data/sl_dependency_v0/raw/tahoe_100m_metadata/gene_metadata.parquet"),
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
