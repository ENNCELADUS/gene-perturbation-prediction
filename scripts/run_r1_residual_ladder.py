...
Loads plain CSV/JSON inputs, runs :func:`aivc_model.residual_ladder.run_r1_ladder`,
and writes ``predictions.csv``, ``per_line.csv``, ``per_gene.csv``, and
``summary.json``. See ``aivc_model.residual_ladder`` for the algorithm, the
...
from aivc_model.residual_ladder import FixedSplit, R1Result, run_r1_ladder
...
def _load_split(path: Path) -> FixedSplit:
    ...
    emptiness) lives in ``aivc_model.residual_ladder`` alongside the rest
