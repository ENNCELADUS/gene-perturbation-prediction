...
def authenticate_uniprot_mapping(
    ...
    if (
        sequence_source.get("uniprot_mapping_json_sha256")
        != sha256_file(mapping_json_path)
        or sequence_source.get("uniprot_mapping_csv_sha256")
    ...
    if any(not isinstance(record, dict) for record in mapping_records) or [
        record.get("gene_symbol") for record in mapping_records
    ] != symbols:
    ...
    for record, csv_record in zip(mapping_records, csv_records, strict=True):
        ...
        if is_resolved:
            ...
            if (
                ...
                or record.get("isoform_identifier")
                != record.get("primary_accession")
