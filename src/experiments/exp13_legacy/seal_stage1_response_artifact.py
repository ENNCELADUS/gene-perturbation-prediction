...
from src.experiments.exp13_legacy.stage1_artifact import seal_stage1_artifact
...
_CODE_RELATIVE_PATHS = {
    "train_geneeffect_response_model.py": Path(
        "src/experiments/exp13_legacy/train_geneeffect_response_model.py"
    ),
    "gene_embeddings.py": Path("src/data/embeddings.py"),
    "response_training.py": Path("src/experiments/exp13_legacy/response_training.py"),
    "stage1_config.py": Path("src/experiments/exp13_legacy/stage1_config.py"),
    "state_core.py": Path("src/model/state.py"),
    "tx1_predicted_response.py": Path(
        "src/experiments/exp13_legacy/tx1_predicted_response.py"
    ),
}
...
def reconstruct_stage1_gene_vocabulary(
    ...
    symbol = next(candidate for candidate, count in path_counts.items() if count == 1)
    genes = [symbol]
...
def _bundle_payload(
    ...
    return {
        ...
        "compatibility_code_paths": {
            name: str(path) for name, path in sorted(compatibility_code_paths.items())
        },
        "config_paths": {
...
def _seal(
    *,
    ...
    bundle = _bundle_payload(compatibility_code_paths, config_paths, source_paths)
    if dry_run:
    ...
    return {
        ...
        "dry_run": dry_run,
        "training_data_provenance_status": (manifest.training_data_provenance_status),
