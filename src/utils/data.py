"""Shared AnnData, pseudobulk, and single-cell helpers."""
...
def validate_artifact_gene_order(
    artifact_gene_names: object,
    data_gene_names: Sequence[str],
    model_name: str,
) -> None:
    """Fail fast when checkpoint score columns do not match evaluation genes."""
    if not isinstance(artifact_gene_names, Sequence) or isinstance(
        artifact_gene_names,
        str,
    ):
        raise ValueError(
            f"{model_name} checkpoint is missing gene order metadata. "
            "Retrain the model or provide a checkpoint with gene_names."
        )

    artifact_names = [str(gene) for gene in artifact_gene_names]
    expected_names = [str(gene) for gene in data_gene_names]
    ...
    mismatch_idx = next(
        (
            idx
            for idx, (artifact_gene, data_gene) in enumerate(
                zip(artifact_names, expected_names)
            )
            if artifact_gene != data_gene
        ),
        min(len(artifact_names), len(expected_names)),
    )
    artifact_gene = artifact_names[mismatch_idx] if mismatch_idx < len(
        artifact_names
    ) else "<missing>"
    data_gene = expected_names[mismatch_idx] if mismatch_idx < len(
        expected_names
    ) else "<missing>"
    raise ValueError(
        f"{model_name} checkpoint gene order does not match evaluation data "
        f"gene order at index {mismatch_idx}: checkpoint={artifact_gene}, "
        f"data={data_gene}. Retrain the model or evaluate with matching data."
    )
...
def build_single_cell_matrices(
    config: Mapping[str, object],
    split_names: Sequence[str] = ("train", "validation", "test"),
) -> dict[str, object]:
    """Create cell-level expression matrices from an AnnData config."""
    data_config = config.get("data_config", {})
    if not isinstance(data_config, Mapping):
        raise ValueError("data_config must be a mapping")
    h5ad_path = data_config.get("h5ad_path")
    ...
    requested_split_names = set(split_names)
    for split_name, split_conditions in splits.items():
        if split_name not in requested_split_names:
            continue
        matrix, valid_conditions, valid_cell_indices = cell_profiles(
            adata=adata,
            conditions=split_conditions,
            condition_key=condition_key,
        )
        matrices[split_name] = matrix
        conditions[split_name] = valid_conditions
        cell_indices[split_name] = valid_cell_indices
    return {
        "gene_names": gene_names,
        "gene_name_to_idx": {gene: idx for idx, gene in enumerate(gene_names)},
        "matrices": matrices,
...
def cell_profiles(
    ...
    """Return one expression profile per cell for requested conditions."""
    requested_conditions = {normalize_condition(condition) for condition in conditions}
    if not requested_conditions:
        return np.empty((0, adata.n_vars), dtype=np.float32), [], []

    obs_conditions = np.asarray(
        [
            normalize_condition(str(value))
            for value in adata.obs[condition_key].tolist()
        ]
    )
    mask = np.isin(obs_conditions, list(requested_conditions))
    if not np.any(mask):
        return np.empty((0, adata.n_vars), dtype=np.float32), [], []
    ...
    if sparse.issparse(matrix):
        matrix = matrix.toarray()
    cell_indices = np.flatnonzero(mask).astype(int).tolist()
    return (
        np.asarray(matrix, dtype=np.float32),
        obs_conditions[mask].astype(str).tolist(),
        cell_indices,
    )
