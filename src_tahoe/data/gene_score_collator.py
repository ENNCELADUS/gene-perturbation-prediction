...
class GeneScoreDataset(Dataset):
    ...
    def __init__(
        ...
        control_layer: Optional[str] = None,
        ...
        self.control_indices = np.array([], dtype=np.int64)
        if self.control_key in self.adata.obs:
            self.control_indices = np.where(self.adata.obs[self.control_key] == 1)[0]

        self.use_control_layer = False
        if len(self.control_indices) == 0:
            if self.control_layer is None:
                raise ValueError(
                    f"No control cells found for control_key='{self.control_key}'. "
                    "Set control_layer to use per-cell control expression."
                )
            if self.control_layer not in self.adata.layers:
                raise ValueError(
                    f"control_layer='{self.control_layer}' not found in adata.layers."
                )
            self.use_control_layer = True

        if (
            self.control_layer is not None
            and self.control_layer not in self.adata.layers
        ):
            raise ValueError(
                f"control_layer='{self.control_layer}' not found in adata.layers."
            )
    ...
    def __getitem__(self, idx: int) -> Dict:
        ...
        if self.use_control_layer:
            ctrl_expr = self._get_control_expression(cell_idx)
            ctrl_binned = self._bin_expression(ctrl_expr)
            control_exprs = np.repeat(
                ctrl_binned[np.newaxis, :], self.n_control_samples, axis=0
            )
        else:
    ...
    def _get_control_expression(self, idx: int) -> np.ndarray:
        if self.control_layer is None:
            raise RuntimeError("control_layer must be set to fetch control expression.")
        row = self.adata.layers[self.control_layer][idx]
        return row.toarray().flatten() if hasattr(row, "toarray") else np.array(row)
    ...
    def _normalize_condition(condition: str) -> str:
        ...
        if text == "ctrl":
            return "ctrl"
        genes = [
            g.strip() for g in text.split("+") if g.strip() and g.strip() != "ctrl"
        ]
        if not genes:
            return "ctrl"
        genes = sorted(genes)
