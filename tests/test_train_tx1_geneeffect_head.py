...
def _write_phase_a_fixture(tmp_path: Path, depmap_path: Path) -> Path:
    ...
    pd.DataFrame(panel_rows).to_csv(phase_a_dir / "k_label_panels.csv", index=False)
...
def _full_fixture(tmp_path: Path) -> dict[str, Path]:
    ...
    phase_a_dir = _write_phase_a_fixture(tmp_path, depmap_path)
...
def test_e2e_smoke_skip_test_predictions_flag(tmp_path: Path) -> None:
